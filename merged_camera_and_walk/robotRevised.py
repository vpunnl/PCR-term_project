...
class Robot:
    ...
    def sense(self,visited):
        ...
        if cautious:
            ...
            for coord in might_validate_coord:
                y = coord[0]
                x = coord[1]
                if self.grid[y][x] in {'+', '?', '!'}: # validation of the "?" and the unexplored area
                    # if self.actual_grid[actual] == 'X': # assume see walls
    ...
    def random_move(self):
        ...
        while True:
            found_winning_pos = ''
            if not directions:
                moves = random.choice([((0, 1),'right'), ((0, -1),'left'), ((1, 0),'down'), ((-1, 0),'up')]) # change to dict and have key value pair
                dy, dx = moves[0]
            ...
            if self.grid[nsy][nsx] not in self.constrain or not directions:
