...
class Robot:
    ...
    def sense(self,visited):
        x, y = self.world.robot_position # actual position on the map
        sx, sy = self.position           # sensed position
        directions = [(0, 1), (0, -1), (1, 0), (-1, 0)]
        cautious = False
        might_validate_coord = []

        for dx, dy in directions:   # magically knows the surroundings
            nx, ny = x + dx, y + dy
            nsx, nsy = sx + dx, sy + dy
            might_validate_coord.append((nsx, nsy))
            if self.world.grid[nx, ny] == 'L':
                cautious = True
            if self.world.grid[nx, ny] == 'W':
                self.grid[nsx, nsy] = 'W'
        ...
        if cautious:
            if not visited:
                for coord in might_validate_coord:
                    ...
                    #     self.sensed_grid[coord] = 'X'
                    if self.grid[coord] == '+':
                        self.grid[coord] = '?'
                    elif self.grid[coord] == '?':
                        self.grid[coord] = '!'
                    elif self.grid[coord] == '!':
                        self.grid[coord] = 'L'
        else: 
            for coord in might_validate_coord:
                if self.grid[coord] in {'+', '?', '!'}: # validation of the "?" and the unexplored area
                    # if self.actual_grid[actual] == 'X': # assume see walls
    ...
    def random_move(self):
        ...
        while True:
            ...
            if self.grid[nsx, nsy] not in self.constrain or not directions:
                if self.grid[nsx,nsy] == '-':
                    visited = True
                ...
                if self.world.grid[nx, ny] == 'W':
                    ...
                    self.world.grid[x, y] = '-'
                    self.grid[sx, sy] = '-'
                    self.world.grid[nx, ny] = 'R'
                    self.grid[nsx, nsy] = 'R'
                    self.world.robot_position = (nx, ny)
                    self.position = (nsx, nsy)
                    print('moved')
                    break
                self.world.grid[x, y] = '-'
                self.grid[sx, sy] = '-'
